...
from app.schemas.newsletter import SubscribeRequest, StatsResponse, UnsubscribeRequest
...
class NewsletterService:
    ...
    async def register(self, data: SubscribeRequest) -> Subscriber:
        ...
        if subscriber:
            ...
            if datetime.now(UTC) < cooldown:
                raise HTTPException(
                    status_code=429, 
                    detail="You must wait 48 hours after unsubscribing to re-subscribe."
                )
    ...
    async def unregister(self, data: UnsubscribeRequest) -> JSONResponse:
